...
class db
{
	...
	function insert($table, $data)
	{
		...
		foreach ($data as $key => $val) {
			$fields[] = '`' . $key . '`';
			$values[] = '\'' . $this->safesql($val) . '\'';
		}
		$sql = 'INSERT INTO `' . PREFIX . '_' . $table . "` (" . implode(', ', $fields) . ') VALUES (' . implode(', ', $values) . ');';
		$this->query($sql);
	}
	...
	function update()
	{
		...
		$items = array();
		foreach ($data as $key => $val) {
			$items[] = '`' . $key . '` = ' . '\'' . $this->safesql($val) . '\'';
		}
		$where = $this->buildQuery($where, $args);
		$sql = 'UPDATE `' . PREFIX . '_' . $table . "` SET " . implode(', ', $items) . ' WHERE ' . $where;
		$this->query($sql);
	}

	protected function buildQuery($sql, $data = array())
	{
		$sql = str_replace("__", PREFIX, $sql);
		$sql = str_replace("%", "%%", $sql);
		$sql = str_replace("?", "%s", $sql);
		foreach ($data as $key => $arg) {
			$data[$key] = "'" . $this->safesql($arg) . "'";
		}
